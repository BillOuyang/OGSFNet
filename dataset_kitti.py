...
class SceneflowDataset(Dataset):
    def __init__(self, root='datasets/kitti_rm_ground', npoints=16384, train=True):
    ...
    def __getitem__(self, index):
        if index in self.cache:
            ...
            pos1_ = torch.from_numpy(np.copy(pos1)[sample_idx1, :]).type(torch.float32)
            pos2_ = torch.from_numpy(np.copy(pos2)[sample_idx2, :]).type(torch.float32)
            flow_ = torch.from_numpy(np.copy(flow)[sample_idx1, :]).type(torch.float32)

        color1 = torch.from_numpy(np.zeros([self.npoints, 3])).type(torch.float32)
        color2 = torch.from_numpy(np.zeros([self.npoints, 3])).type(torch.float32)
        mask = torch.from_numpy(np.ones([self.npoints])[:, np.newaxis]).type(torch.float32)
...
if __name__ == '__main__':
    ...
    d = SceneflowDataset(root='F3D_data/kitti_rm_ground', npoints=16384)
    print(len(d))
    import time
    tic = time.time()
